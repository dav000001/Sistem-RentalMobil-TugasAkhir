...
@section('title', 'Laporan ' . $complaint->reference)

@section('content')
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">

    <div class="mb-6">
        <a href="{{ route('vendor.my-reports.index') }}" class="text-sm text-blue-600 hover:underline">← Laporan Saya</a>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
                ...
                @if($complaint->demanded_refund_amount > 0)
                    <div class="mt-4 pt-4 border-t">
                        <div class="bg-orange-50 border border-orange-200 rounded-lg px-4 py-3">
                            <p class="text-xs text-gray-500 mb-0.5">💰 Estimasi Biaya Perbaikan yang Anda Ajukan</p>
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-6">
                    ...
                    <div class="grid grid-cols-2 gap-3 text-sm">
                        ...
                        <div>
                            ...
                            <p class="font-medium">{{ $complaint->booking->customer?->full_name ?? '-' }}</p>
                        </div>
                        <div>
            ...
            {{-- Info tagihan kompensasi jika sudah dibuat --}}
            @php
                $compensation = \App\Models\CompensationCharge::where('booking_id', $complaint->booking_id)
                    ->whereIn('status', ['pending', 'paid'])
                    ->first();
            @endphp
            @if($compensation)
                <div class="rounded-xl border p-5
                    {{ $compensation->status === 'paid' ? 'bg-green-50 border-green-200' : 'bg-orange-50 border-orange-200' }}">
                    <h2 class="font-semibold mb-2 {{ $compensation->status === 'paid' ? 'text-green-800' : 'text-orange-800' }}">
                        {{ $compensation->status === 'paid' ? '✅ Kompensasi Sudah Dibayar Customer' : '⏳ Tagihan Kompensasi Dikirim ke Customer' }}
                    </h2>
                    <p class="text-sm {{ $compensation->status === 'paid' ? 'text-green-700' : 'text-orange-700' }}">
                        Jumlah: <strong>Rp {{ number_format($compensation->amount, 0, ',', '.') }}</strong>
                    </p>
                    <p class="text-xs {{ $compensation->status === 'paid' ? 'text-green-600' : 'text-orange-600' }} mt-1">
                        {{ $compensation->reason }}
                    </p>
                    @if($compensation->status === 'paid' && $compensation->paid_at)
                        <p class="text-xs text-green-500 mt-1">Dibayar pada {{ $compensation->paid_at->format('d M Y H:i') }}</p>
                    @elseif($compensation->status === 'pending' && $compensation->due_date)
                        <p class="text-xs text-orange-500 mt-1">Batas bayar customer: {{ $compensation->due_date->format('d M Y') }}</p>
                    @endif
                </div>
            @endif
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6">
                ...
                @if($publicResponses->isEmpty())
                    <p class="text-gray-500 text-sm">Belum ada respons dari admin.</p>
                @else
                    <div class="space-y-4">
                ...
                @if($complaint->isOpen())
                    <div class="mt-6 pt-6 border-t">
                        <h3 class="font-medium text-sm mb-2">Tambah Keterangan / Respons</h3>
                        <p class="text-xs text-gray-500 mb-3">Kirim informasi tambahan ke admin terkait laporan ini.</p>
                        <form method="POST" action="{{ route('vendor.my-reports.respond', $complaint->id) }}">
                            @csrf
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-5">
                <h3 class="font-semibold mb-4 text-sm">Riwayat Status</h3>
                <div class="space-y-3">
